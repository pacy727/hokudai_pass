...
export class ReviewQuestionRequestService {
  ...
  // 学習記録IDで復習問題リクエストを取得（追加）
  static async getRequestsByStudyRecordId(studyRecordId: string): Promise<ReviewQuestionRequest[]> {
    try {
      console.log('🔍 Fetching requests for studyRecordId:', studyRecordId);
      ...
      console.log('✅ Requests for studyRecordId loaded:', requests.length);
      return requests;
    } catch (error) {
      console.error('❌ Error fetching requests by studyRecordId:', error);
      return [];
    }
  }
